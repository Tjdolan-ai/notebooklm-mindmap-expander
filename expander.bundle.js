/**
 * NotebookLM Mind Map Auto‑Expander (ES2022 bundle)
 * Quick‑compiled from src/expander.ts
 */
 (() => {
  ...
  function collapseAll() {
    if (!ensureContainer())
      return;
    if (tryToolbarCollapse())
      return;
    walkAndToggle(COLLAPSE_LABEL);
  }
  function ensureContainer() {
    if (lastContainer && document.contains(lastContainer))
      return true;
  ...
  function injectToggle() {
    if (document.getElementById("nlm-expander-toggle"))
      return;
    if (!ensureContainer())
      return;
